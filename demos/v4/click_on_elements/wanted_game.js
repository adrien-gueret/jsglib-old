(window => {
    const { Game, Utils} = window.JSGLib.Core;
    const Head = window.Head;
    const { BigHeadsSprite, CHARACTERS_NUMBERS } = window.WantedGameSprites;

    class WantedGame extends Game {
        ...
        updateTimer() {
            const timer = this.time_counter >= 10 ? this.time_counter : '0' + this.time_counter;
        ...
        generateHeads() {
            ...
            const numbers = [this.wanted_tile.tile_number];
            ...
            Utils.shuffleArray(numbers).forEach(number => {
                const head = new Head(this.current_room.getSize(), number);
