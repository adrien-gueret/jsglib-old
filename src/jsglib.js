...
(function (window, document, undefined) {
    ...
    JSGlib = {
        ...
        /// <field name="getVectorDirection" static="true" type="Function">Return the direction of the given vectors.</field>
        getVectorDirection: function (vectorValue1, vectorValue2) {
            /// <param name="vectorValue1" type="Number">Value of first vector (typically hspeed)</param>
            /// <param name="vectorValue1" type="Number">Value of second vector (typically vspeed).</param>
            /// <returns type="Number">Calculated direction, in degree.</returns>
            var direction   =   Math.atan2(-vectorValue2, vectorValue1) / (Math.PI / 180);
        ...
        ajax: function (options) {
			...
			///  'onComplete': function () { Function called after request is finished, after onError() or onSuccess() }<br />
            /// }</param>
			/// <returns type="JSGlib">The JSGlib object.</returns>
        ...
        Game: function (idContainer, tilesSize) {
                        ...
                        if (this.animatedTiles.length > 0) {
                            ...
                            JSGlib.forEach(this.animatedTiles, function () {
                                ...
                                if (t !== null) {
                                    ...
                                    layer.tiles[this.sheetName][x][y] = this.sprite.getCurrentTileNumber();
                                    layer.drawTile(x, y, this.sheetName);
                                }
                            });
                        }
                    });
					...
					JSGlib.forEach(globalCollision, function (k) {
						...
						if (!hasMask) {
							...
							JSGlib.forEach(masks, function () {
                                ...
                                if (maskRectangle.isCollidedWithRectangle(tileRectangle)) {
									if (performThisTile() == JSGlib.KEYWORDS.BREAK) {
										return JSGlib.KEYWORDS.BREAK;
									}
                                ...
                                JSGlib.forEach(json, function (i) {
                                        ...
                                        initTile = function (key) {
                                            ...
                                            JSGlib.forEach(tileDef.types, function (j) {
                                                ...
                                                JSGlib.forEach(this, function () {
                                                    _tiles[key].types[this] = jsglib_types_tiles[typeName];
                                                });
                                            });
									...
									(function (key) {
										if (imgHTML.complete) {
											...
											imgHTML.addEventListener('load', function() {initTile(key);});
										}
									})(i.key);
                                });
								numLoaded++;
            ...
            this.createLayer = function (name, willContainElements) {
                ...
                _layers.push({
                    ...
                    drawTile: function (x, y, name) {
						var index = this.tiles[name][x][y],
                            spritePosition = new JSGlib.Point(0, 0),
                            abs = y * _tilesSize,
                            ord = x * _tilesSize;

                        if (index > 0) {
                            this.spritesTiles[name].usedTiles([index]);
                            spritePosition.copy(this.spritesTiles[name].getCurrentTilePosition());

                            this.ctx.clearRect(abs, ord, _tilesSize, _tilesSize);
                            this.ctx.drawImage(this.spritesTiles[name].getImage(), spritePosition.x(), spritePosition.y(),
				...
				JSGlib.forEach(_layers, function () {
					...
					for (x = xMin; x <= xMax; x++) {
						for (y = yMin; y <= yMax; y++) {
							...
							JSGlib.forEach(thisLayer.tiles, function (i) {
								if (this[y] && this[y][x]) {
								
								var tileType = _tiles[i.key].types[this[y][x]],
									tileX,
									tileY;
								if (tileType) {
            ...
            this.getTileType = function (x, y, layerName, sheetName) {
                ...
                /// <param name="sheetName" type="String" optional="true">Sheet name to check on given layer. If not specified only first registered sheet will be checked.</param>
				...
				if(sheetName) {
					if (layer.tiles[sheetName] && layer.tiles[sheetName][y] && layer.tiles[sheetName][y][x]) {
						return _tiles[sheetName].types[layer.tiles[sheetName][y][x]];
                    }
					return 0;
				}
				...
				JSGlib.forEach(layer.tiles, function (i) {
					if (this[y] && this[y][x]) {
                        type = _tiles[i.key].types[this[y][x]];
						return JSGlib.KEYWORDS.BREAK;
                    }
				});
            ...
            this.run = function () {
                ...
                if (!_isRunning) {
                    ...
                    _container.addEventListener('mousemove', function (e) {
						...
						x -= (this.getBoundingClientRect().left  + (window.pageXOffset || this.scrollLeft)  - (this.clientLeft  || 0));
						y -= (this.getBoundingClientRect().top  + (window.pageYOffset || this.scrollTop)  - (this.clientTop  || 0));
                ...
                if (definition) {
                    ...
                    JSGlib.forEach(definition.tiles, function (layerDef) {
                        ...
                        JSGlib.forEach(this, function (tileDef) {
                            var sheetName = tileDef.key,
                                tilesDef = _tiles[sheetName];
						
                            if (tilesDef.sprite) {        
								layer.tiles[sheetName] = this;
                                layer.spritesTiles[sheetName] = tilesDef.sprite;
								
								JSGlib.forEach(this, function (i) {
									JSGlib.forEach(this, function (j) {
										var index = this,
											animatedSprite = null,
											indexAnimation = _thisGame.isTileAnimated(sheetName, index),
											animationSpeed = 0;
										layer.drawTile(i.key, j.key, sheetName);
										...
										if (indexAnimation !== false) {
											...
											if (animationSpeed) {
												animatedSprite = new JSGlib.Sprite(layer.spritesTiles[sheetName].getImage())
            ...
            this.getInstanceNearest = function (point, className, excludeInstance, maxDistance) {
                ...
                /// <param name="maxDistance" type="Integer" optional="true">If specified, found instance will be returned only if the distance between it and point is less than given value.</param>
                ...
                JSGlib.forEach(_activeElements, function () {
                    ...
                    if ((minValue < 0 || tempValue < minValue) && (!maxDistance || maxDistance && maxDistance >= tempValue)) {
                        minValue = tempValue;
                        element = this;
                    }
                });
        ...
        Element: function (x, y, layer, game) {
            ...
            this.instanceNearest = function (className, maxDistance) {
                /// <summary>Get the active instance which center is the nearest of the center of this element.</summary>
                /// <param name="className" type="Bool/String" optional="true">If not false, this method will check only instances of given type. Default: false.</param>
                /// <param name="maxDistance" type="Integer" optional="true">If specified, found instance will be returned only if the distance between it and called instance is less than given value.</param>
                /// <returns type="JSGlib.Element">The found Element, or null if none has been found.</returns>

                return _game.getInstanceNearest(this.getCenter(), className, this, maxDistance);
            };
